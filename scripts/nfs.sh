...
sudo chown -R sas:sas $nfsMountDirectory


echo $thisHost | grep -q "${gridNodeHostnamePrefix}1"
if [ $? -eq 0 ]; then

  # These can be created only after the mount of FSS-NFS
  ...
  echo "downloading depot to NFS FSS"
  cd $nfsMountDirectory/SASDEPOT/
  ...
  tarOptions="$nfsMountDirectory/SASDEPOT/$sasDepotFilename -C $nfsMountDirectory/SASDEPOT/ --exclude=${sasDepotRoot}/sid_files/LSF94_*.txt --exclude=${sasDepotRoot}/sid_files/SAS94_*.txt"
  tar xvzf $tarOptions
  while [ $? -ne 0 ]; do
    rm -rf ${sasDepotRoot}
    tar xvzf $tarOptions
  ...
  cp $sasDepotPlanFilename $nfsMountDirectory/SASDEPOT/${sasDepotRoot}/plan_files/
  chmod 644 -R $nfsMountDirectory/SASDEPOT/${sasDepotRoot}/plan_files
  chown sas:sas -R  $nfsMountDirectory/SASDEPOT/${sasDepotRoot}/plan_files
  ...
  cp $sasDepotPlanFilename $nfsMountDirectory/SASDEPOT/${sasDepotRoot}/
  chmod 644 $nfsMountDirectory/SASDEPOT/${sasDepotRoot}/plan.xml
  chown sas:sas $nfsMountDirectory/SASDEPOT/${sasDepotRoot}/plan.xml
